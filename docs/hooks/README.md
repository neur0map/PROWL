...
Hooks are commands that Prowl runs when a chosen event happens. They let you
set firm local rules without asking the model to remember those rules.

## What hooks can do

- Block a command you never want Prowl to run.
- Change tool input before the tool sees it.
- Add a short note when a certain tool is used.
- Allow a trusted tool without another prompt.
- Keep a local record of tool calls.

A hook may be written in any language available on the machine. Hooks run at
the same time when possible, then Prowl applies their answers in config order.
Prowl currently supports `PreToolUse`.

## A first hook

This example blocks Haskell tools.

### Config

The first thing we need to do is hook up our hook. Let's add the following to
our **project-level** `prowl.json`. Relative paths like `./no-haskell.sh` work
here because the project root is your working directory. If you're configuring
a global hook (`~/.config/prowl/prowl.json`), use an absolute path instead.
...
# ($PROWL_TOOL_INPUT_COMMAND) to grep and match on a regexp.
if echo "$PROWL_TOOL_INPUT_COMMAND" | grep -qE '(^| )((ghc|cabal|stack)(\.exe)?)( |$)'; then
...
Hooks run through Prowl's embedded POSIX shell (`mvdan.cc/sh`) — the same
...
- **Permissive shebang fallback**: if the absolute path in a shebang
  doesn't exist (e.g. `#!/bin/bash` on Windows), Prowl falls back to a
  ...
  "no opinion".
- **Environment**: every hook sees `PROWL=1`, `AGENT=prowl`, and
  `AI_AGENT=prowl` on top of the `PROWL_*` hook-specific variables. These
  ...
  both contexts.
- **Timeout behavior**: when a hook exceeds its timeout, Prowl cancels the
  context and waits a short grace period (~1s) for the interpreter to
  yield. If the hook still hasn't returned, Prowl abandons it, logs a
...
Hooks can be added to your `prowl.json` (or `.prowl.json`) at both the global
...
> work fine in project-level `prowl.json` because the project root is also
> your working directory. For **global** config (`~/.config/prowl/`),
> however, you must use either an absolute path or an inline command:
>
> ```jsonc
> // Global ~/.config/prowl/prowl.json
> {
>   "hooks": {
>     "PreToolUse": [
>       {
>         "command": "/home/you/.config/prowl/hooks/no-haskell.sh"
...
3. Runs all matching hooks **in parallel** through Prowl's embedded POSIX
...
5. Applies the result **before** permission checks. If the aggregated decision
   ...
   for it. If it's `allow`, Prowl treats that as affirmative pre-approval and
...
however you'll incur some additional overhead as Prowl will still parse and
run each hook.
...
| ---------------------------- | ---------------------------------------------- |
| `PROWL`                      | Always `1` when running under Prowl.           |
| `AGENT`                      | Always `prowl`.                                |
| `AI_AGENT`                   | Always `prowl`.                                |
| `PROWL_EVENT`                | The hook event name (e.g. `PreToolUse`).       |
| `PROWL_TOOL_NAME`            | The tool being called (e.g. `bash`).           |
| `PROWL_SESSION_ID`           | Current session ID.                            |
| `PROWL_CWD`                  | Working directory.                             |
| `PROWL_PROJECT_DIR`          | Project root directory.                        |
| `PROWL_TOOL_INPUT_COMMAND`   | For `bash` calls: the shell command being run. |
| `PROWL_TOOL_INPUT_FILE_PATH` | For file tools: the target file path.          |

The `PROWL`, `AGENT`, and `AI_AGENT` markers are also set by the `bash`
tool, so a script can detect "am I running under Prowl?" the same way in
either context.
...
Hooks communicate back to Prowl via **exit code** and `stdout`/`stderr`. The
...
deterministic based on the order hooks appear in `prowl.json`.
...
If a hook exceeds its timeout, Prowl cancels its context and treats the
...
if echo "$PROWL_TOOL_INPUT_COMMAND" | grep -qE 'rm\s+-(rf|fr)\s+/'; then
...
returns `decision: "allow"`, which tells Prowl to pre-approve the call:
...
case "$PROWL_TOOL_INPUT_COMMAND" in
...
if [[ "$PROWL_TOOL_INPUT_FILE_PATH" == *.go ]]; then
...
```jsonc
{ "command": "echo \"$(date -Iseconds) $PROWL_TOOL_NAME\" >> ./tools.log" }
```
...
Prowl hooks are broadly compatible with [Claude Code
...
hooks run under Prowl unchanged. This document covers the Prowl-specific API
only — anything not documented here isn't guaranteed to work.

One intentional divergence: Prowl treats `updated_input` as a shallow-merge
...
4. `decision` precedence: `deny` > `allow` > `null`. First deny determines the
   ...
   is `allow`, Prowl pre-approves the tool call and skips the permission
...
Open a question or send a change through the
[Prowl repository](https://github.com/neur0map/PROWL).
