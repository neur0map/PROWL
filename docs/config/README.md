...
Prowl can update this file through its built-in config skill. You can describe
the change you want, or edit the file yourself. The same skill can convert an
older JSON config.

Prowl is configured with Bash via a set of Prowl-specific builtin commands. By
default, global config lives at `~/.config/prowl/prowlrc` on Unix-like systems
and `%USERPROFILE%\.config\prowl\prowlrc` on Windows. It works like a `.bashrc`:
it runs when Prowl starts and configures the agent.
...
1. Prowl ships with a first-class Bash interpreter, so we get the logic for
   free.
2. Ultimately, Prowl needs to be able to configure itself, and command-based
...
target specific Prowl versions with `$PROWL_VERSION`:

```bash
if [[ $PROWL_VERSION == "0.85.*" ]]; then
    option debug true
fi
```
...
Just like `prowl.json`, `prowlrc` is a trusted file. Guard it carefully and
don't download random configs without reading them first.

## Where config lives

Prowl looks for config in the following places, with lower numbers taking
precedence:
...
| 1        | `./.prowlrc`                     | `.\.prowlrc`                      |
| 2        | `./prowlrc`                      | `.\prowlrc`                       |
| 3        | `$XDG_CONFIG_HOME/prowl/prowlrc` | `%XDG_CONFIG_HOME%\prowl\prowlrc` |

Legacy JSON uses `.prowl.json` / `prowl.json` in the same directories as the
above. Everything found is merged, with project settings overriding global ones
and `prowlrc` overriding JSON in the same directory. If a folder has both, they
merge and Prowl logs a warning.

Data directories (`~/.local/share/prowl` on Unix-like systems and
`%LOCALAPPDATA%\prowl` on Windows) contain machine-owned JSON state. Prowl does
not discover or execute a `prowlrc` from those locations.

> [!NOTE]
> Prowl also stores state data in `$XDG_DATA_HOME/prowl`
> (`%LOCALAPPDATA%\prowl` on Windows). This is application state, and should
...
Available Commands:
  ...
  option        Configure general Prowl behavior
```
...
same `<provider>/<id>` form printed by `prowl models`.
...
Configure general Prowl behavior, paths, attribution, and the terminal UI.
...
Boolean Keys:
  ...
  attribution-generated-with     add the Generated with Prowl line

String Keys:
  data-directory string            directory for project data and state
  initialize-as string             context filename created by prowl init
...
Available Keys:
  ...
  exit-banner default|compact|none
                                ...
                                the Prowl logo, compact shows only the resume
...
> for them: `.agents/skills`, `.prowl/skills`, `.claude/skills`,
> `.cursor/skills`.
...
```bash
# Unix-like: ~/.config/prowl/prowlrc
# Windows:   %USERPROFILE%\.config\prowl\prowlrc
source ~/team/prowl-base.sh    # sets up providers, a few skills
...
`prowl.json` is the original format and is now deprecated. We plan to support
...
headers) are shell-expanded at load time. In `prowlrc` there's no such list —
it's all just Bash.

Both formats are trusted code: they run with your shell privileges before the UI
appears. Don't launch Prowl in a directory whose config you haven't read.
