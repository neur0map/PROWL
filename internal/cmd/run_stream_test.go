...
// with reason=tool_use and used to terminate `prowl run` early via
...
// subscribers (e.g. a TUI watching workspace events while `prowl
...
// non-nil error from `prowl run` so shells and CI catch it via
// exit status.
...
// run (e.g. Ctrl+C while `prowl run` waits) exits cleanly rather
...
// queue scenario: `prowl run --continue` attaches to a session
