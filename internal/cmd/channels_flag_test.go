...
// one. When local, `prowl run --channels server:webhook` fails with
...
func TestChannelsFlagAvailableOnRunCmd(t *testing.T) {
	...
	require.NotNil(t, flag, "the --channels flag must be available on `prowl run` (register it as a persistent flag on rootCmd)")
...
func TestChannelsFlagAvailableOnRootCmd(t *testing.T) {
	...
	require.NotNil(t, flag, "the --channels flag must be available on `prowl` (rootCmd)")
}
