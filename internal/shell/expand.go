...
// to re-enable strict mode globally. Not exposed in prowl.json; this is
...
func ExpandValue(ctx context.Context, value string, env []string) (string, error) {
	...
	// verbatim, with no PROWL/AGENT/AI_AGENT injection: callers of
	...
	cfg := &expand.Config{
		...
		CmdSubst: func(w io.Writer, cs *syntax.CmdSubst) error {
			...
			runnerOpts := []interp.RunnerOption{
				...
				interp.Dir(s.cwd),
				execHandlerOption(s.blockFuncs, s.guards),
			}
			if strict {
