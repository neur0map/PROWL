...
type RunOptions struct {
	...
	// the Prowl process cwd — hooks and the bash tool have different
...
// standard Prowl middleware chain (builtins, script dispatch, block list)
...
// regardless of the caller's environment because Prowl shells are never
...
//  1. builtins first (so Prowl's in-process jq wins over any PATH binary);
...
// builtinHandler returns middleware that dispatches recognized Prowl
