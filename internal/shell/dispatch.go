...
func scriptDispatchHandler(blockFuncs []BlockFunc, guards []Guard) execMiddleware {
	return func(next interp.ExecHandlerFunc) interp.ExecHandlerFunc {
		return func(ctx context.Context, args []string) error {
			...
			case isBinary(probe):
				return next(ctx, args)
			default:
				return runShellSource(ctx, scriptPath, args, blockFuncs, guards)
			}
		}
	}
}
...
func dispatchShebang(ctx context.Context, scriptPath string, probe []byte, args []string) error {
	...
	if err != nil {
		...
		fmt.Fprintf(hc.Stderr, "prowl: %s: %s\n", scriptPath, err)
...
// env, and stdio, and rebuilds the Prowl handler stack so builtins and the
...
func runShellSource(ctx context.Context, path string, args []string, blockFuncs []BlockFunc, guards []Guard) error {
