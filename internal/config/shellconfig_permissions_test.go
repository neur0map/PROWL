...
// loadProwlSh writes a prowl.sh into an isolated project and loads it through
...
func loadProwlSh(t *testing.T, script string) *config.ConfigStore {
	t.Helper()
	store, err := loadProwlShErr(t, script)
	require.NoError(t, err)
	return store
}

// loadProwlShErr is loadProwlSh without asserting success, for cases that are
// expected to fail at load time.
func loadProwlShErr(t *testing.T, script string) (*config.ConfigStore, error) {
	t.Helper()
	...
	workDir := t.TempDir()
	dataDir := t.TempDir()
	require.NoError(t, os.WriteFile(filepath.Join(workDir, "prowlrc"), []byte(script), 0o644))

	return config.Load(workDir, dataDir, false)
}

func TestShellConfigPermissionsAllow(t *testing.T) {
	store := loadProwlSh(t, `permissions allow bash view`)
...
func TestShellConfigPermissionsAccumulateAndDedup(t *testing.T) {
	store := loadProwlSh(t, `permissions allow bash
...
func TestShellConfigPermissionsDeny(t *testing.T) {
	store := loadProwlSh(t, `permissions deny bash sourcegraph
