...
// isolateReloadEnv points HOME/XDG/PROWL_* at a throwaway directory so a
...
func isolateReloadEnv(t *testing.T) (workDir, dataDir string) {
	...
	t.Setenv("PROWL_GLOBAL_CONFIG", filepath.Join(isolated, ".config", "prowl"))
	t.Setenv("PROWL_GLOBAL_DATA", filepath.Join(isolated, ".local", "share", "prowl"))
	return t.TempDir(), t.TempDir()
}

// TestReloadFromDisk_PicksUpEditedProwlrc verifies that a prowlrc edit on
// disk is reflected in memory after a reload, matching how JSON config
// already behaves.
func TestReloadFromDisk_PicksUpEditedProwlrc(t *testing.T) {
	workDir, dataDir := isolateReloadEnv(t)
	rcPath := filepath.Join(workDir, "prowlrc")
	...
	require.Equal(t, "osc", store.Config().Options.Notifications,
		"reload must re-execute the prowlrc and pick up the new value")
}

// TestReloadFromDisk_FailingProwlrcKeepsOldConfig verifies that a prowlrc
...
func TestReloadFromDisk_FailingProwlrcKeepsOldConfig(t *testing.T) {
	workDir, dataDir := isolateReloadEnv(t)
	rcPath := filepath.Join(workDir, "prowlrc")
	require.NoError(t, os.WriteFile(rcPath, []byte("option notifications bell\n"), 0o644))
	...
	// Overwrite with a prowlrc that errors at load time (unknown option key).
	require.NoError(t, os.WriteFile(rcPath, []byte("option totally-bogus-key value\n"), 0o644))

	err = store.ReloadFromDisk(context.Background())
	require.Error(t, err, "a failing prowlrc must surface a reload error")
	require.Equal(t, "bell", store.Config().Options.Notifications,
		"in-memory config must be preserved when a reload fails")
}

// TestReloadFromDisk_HangingProwlrcIsInterruptible is the core regression test
// for the reload-path blocker: config reloads run while holding the store's
// write lock, so a prowlrc that blocks (busy loop, hung command substitution)
...
func TestReloadFromDisk_HangingProwlrcIsInterruptible(t *testing.T) {
	workDir, dataDir := isolateReloadEnv(t)
	rcPath := filepath.Join(workDir, "prowlrc")
	require.NoError(t, os.WriteFile(rcPath, []byte("option notifications bell\n"), 0o644))
	...
	// Overwrite with a prowlrc that never terminates.
...
// TestLoad_TracksNotYetCreatedGlobalProwlrc verifies that config.Load tracks
// the global prowlrc path even when the file does not exist yet, so a prowlrc
// created after startup is detected as a staleness change. Previously only
// successfully-loaded paths were tracked, so a mid-session global prowlrc went
// unnoticed until something else triggered a reload.
//
// Project-level prowlrc files are discovered by walking the tree for existing
// files, so a not-yet-created project prowlrc is still only picked up on the
// next reload; the global path is the common case this covers.
func TestLoad_TracksNotYetCreatedGlobalProwlrc(t *testing.T) {
	workDir, dataDir := isolateReloadEnv(t)
	globalRC := filepath.Join(t.TempDir(), "prowlrc")
	t.Setenv("PROWL_GLOBAL_CONFIG", filepath.Dir(globalRC))
