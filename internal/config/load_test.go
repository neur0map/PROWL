...
func TestLookupConfigs_BoundedByProject(t *testing.T) {
	...
	globalDir := t.TempDir()
	dataDir := t.TempDir()
	t.Setenv("PROWL_GLOBAL_CONFIG", globalDir)
	t.Setenv("PROWL_GLOBAL_DATA", dataDir)

	t.Run("does not pick up prowl.json above non-git project", func(t *testing.T) {
		parent := t.TempDir()

		// prowl.json above the project must not be adopted.
		require.NoError(t, os.WriteFile(
			filepath.Join(parent, "prowl.json"),
			[]byte(`{}`),
			0o644,
		))
		...
		for _, p := range got {
			require.NotEqual(t, filepath.Join(parent, "prowl.json"), p)
		}
	})

	t.Run("does not climb out of git worktree to find prowl.json", func(t *testing.T) {
		...
		strayEval, err := filepath.EvalSymlinks(filepath.Join(parent, "prowl.json"))
		require.NoError(t, err)
		for _, p := range got {
			...
			require.NotEqual(t, strayEval, pEval, "must not adopt parent prowl.json")
		}
	})

	t.Run("picks up prowl.json inside the project", func(t *testing.T) {
		project := t.TempDir()
		local := filepath.Join(project, "prowl.json")
		...
		require.True(t, foundLocal, "expected project prowl.json to be in lookup result: %v", got)
	})
	...
	t.Run("global shell config (prowlrc) is included", func(t *testing.T) {
		project := t.TempDir()

		got := lookupConfigs(project)
		// A global prowlrc is discovered only beside the user config. The data
		// directory is machine-owned state and must never execute a prowlrc.
		require.Contains(t, got, shellConfigSibling(GlobalConfig()))
		require.NotContains(t, got, shellConfigSibling(GlobalConfigData()))
	})

	t.Run("project prowlrc and .prowlrc are discovered", func(t *testing.T) {
		project := t.TempDir()
		require.NoError(t, os.WriteFile(filepath.Join(project, "prowlrc"), []byte(""), 0o644))
		require.NoError(t, os.WriteFile(filepath.Join(project, ".prowlrc"), []byte(""), 0o644))

		got := lookupConfigs(project)
		require.Contains(t, got, filepath.Join(project, "prowlrc"))
		require.Contains(t, got, filepath.Join(project, ".prowlrc"))
	})
	...
	t.Run("system config is loaded first", func(t *testing.T) {
		...
		require.Equal(t, "/etc/prowl/prowl.json", got[0])
	})
}
...
// config and a prowlrc coexist in the same directory, the merge warning names
...
func TestLoadFromConfigPaths_ConflictWarningNamesKeys(t *testing.T) {
	...
	t.Run("names overlapping keys", func(t *testing.T) {
		buf := capture(t)
		tmpDir := t.TempDir()
		jsonPath := filepath.Join(tmpDir, "prowl.json")
		rcPath := filepath.Join(tmpDir, "prowlrc")
		...
		require.NoError(t, err)
		require.Contains(t, buf.String(), "prowlrc taking precedence")
	...
	t.Run("no warning when nothing overlaps", func(t *testing.T) {
		...
		_, _, err := loadFromConfigPaths(context.Background(), []string{jsonPath, rcPath})
		require.NoError(t, err)
		require.NotContains(t, buf.String(), "prowlrc taking precedence",
...
func TestConfig_setDefaults(t *testing.T) {
	t.Run("sets default data directory", func(t *testing.T) {
		...
		require.Equal(t, filepath.Join(workingDir, ".prowl"), cfg.Options.DataDirectory)
	...
	t.Run("does not adopt .prowl from a parent project", func(t *testing.T) {
		parent := t.TempDir()

		// .prowl in the parent: it should not be reused by the child
	...
	t.Run("does not climb out of git worktree to find .prowl", func(t *testing.T) {
		if _, err := exec.LookPath("git"); err != nil {
			t.Skip("git not available")
		}
		...
		// Stray .prowl above the worktree root.
		require.NoError(t, os.Mkdir(filepath.Join(parent, defaultDataDirectory), 0o755))
		...
		require.NoError(t, err)
		require.NotEqual(t, strayEval, gotEval, "must not adopt parent .prowl")
...
func TestConfig_setupAgentsWithNoDisabledTools(t *testing.T) {
	...
	require.True(t, ok)
	assert.Equal(t, []string{"lsp_symbols", "lsp_definition", "lsp_call_hierarchy", "glob", "grep", "ls", "sourcegraph", "view", "prowl_agent"}, taskAgent.AllowedTools)
}
...
func TestConfig_setupAgentsWithDisabledTools(t *testing.T) {
	...
	assert.Equal(t, []string{"agent", "bash", "prowl_info", "prowl_logs", "job_output", "job_kill", "multiedit", "lsp_diagnostics", "lsp_references", "lsp_restart", "lsp_symbols", "lsp_definition", "lsp_call_hierarchy", "lsp_rename", "lsp_replace_symbol", "fetch", "agentic_fetch", "glob", "ls", "question", "sourcegraph", "todos", "view", "write", "list_mcp_resources", "read_mcp_resource", "prowl_agent", "manage_skill", "learn"}, coderAgent.AllowedTools)

	taskAgent, ok := cfg.Agents[AgentTask]
	require.True(t, ok)
	assert.Equal(t, []string{"lsp_symbols", "lsp_definition", "lsp_call_hierarchy", "glob", "ls", "sourcegraph", "view", "prowl_agent"}, taskAgent.AllowedTools)
}
...
func TestConfig_setupAgentsWithEveryReadOnlyToolDisabled(t *testing.T) {
	...
	coderAgent, ok := cfg.Agents[AgentCoder]
	require.True(t, ok)
	assert.Equal(t, []string{"agent", "bash", "prowl_info", "prowl_logs", "job_output", "job_kill", "download", "edit", "multiedit", "lsp_diagnostics", "lsp_references", "lsp_restart", "lsp_rename", "lsp_replace_symbol", "fetch", "agentic_fetch", "question", "todos", "write", "list_mcp_resources", "read_mcp_resource", "manage_skill", "learn"}, coderAgent.AllowedTools)
...
func TestConfig_setDefaultsDisableDefaultProvidersEnvVar(t *testing.T) {
	t.Run("sets option from environment variable", func(t *testing.T) {
		t.Setenv("PROWL_DISABLE_DEFAULT_PROVIDERS", "true")
...
func TestConfig_configureSelectedModels(t *testing.T) {
	t.Run("reload mode should not persist fallback defaults", func(t *testing.T) {
		dir := t.TempDir()
		globalPath := filepath.Join(dir, "prowl.json")
