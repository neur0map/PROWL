...
func TestConfigStore_ConfigPath_GlobalAlwaysWorks(t *testing.T) {
	...
	store := &ConfigStore{
		globalDataPath: "/some/global/prowl.json",
	}

	path, err := store.configPath(ScopeGlobal)
	require.NoError(t, err)
	require.Equal(t, "/some/global/prowl.json", path)
}
...
func TestConfigStore_ConfigPath_WorkspaceReturnsPath(t *testing.T) {
	...
	store := &ConfigStore{
		workspacePath: "/some/workspace/.prowl/prowl.json",
	}

	path, err := store.configPath(ScopeWorkspace)
	require.NoError(t, err)
	require.Equal(t, "/some/workspace/.prowl/prowl.json", path)
}
...
func TestGlobalWorkspaceDir(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("PROWL_GLOBAL_DATA", dir)
...
func TestReloadFromDisk_UsesNewConfigValues(t *testing.T) {
	...
	t.Setenv("PROWL_GLOBAL_CONFIG", dir)
	t.Setenv("PROWL_GLOBAL_DATA", dir)
	resetProviderState()
