...
func TestShellConfigLSPAdd(t *testing.T) {
	store := loadProwlSh(t, `lsp add gopls --command gopls --filetypes go --filetypes mod --root-markers go.mod --timeout 60`)
...
func TestShellConfigLSPRemove(t *testing.T) {
	store := loadProwlSh(t, `lsp add keepls --command keep-server
