...
func isolateHyperCredentials(t *testing.T) {
	...
	t.Setenv("PROWL_HYPER_API_KEY", "")
}

// writeTokenToDisk persists token as the hyper provider credential in the
// config file at path, mimicking what another prowl instance would leave
...
// which lets a single test process faithfully simulate two prowl instances:
...
func TestRefreshOAuthToken_InProcessSingleFlight(t *testing.T) {
	...
	configPath := filepath.Join(t.TempDir(), "prowl.json")
