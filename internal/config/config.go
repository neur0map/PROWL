...
type ProviderConfig struct {
	...
	// When Models is empty and this is nil or true, Prowl auto-discovers
...
type MCPConfig struct {
	...
	Sessionless *bool `json:"sessionless,omitempty" jsonschema:"description=Mark a sessionless MCP server (no Mcp-Session-Id) so Prowl skips the subscriptions/listen stream it would otherwise reject. Leave unset to auto-detect known sessionless servers (e.g. GitHub MCP),default=false"`
...
type TUIOptions struct {
	...
	ExitBanner  ExitBanner  `json:"exit_banner,omitempty" jsonschema:"description=Exit banner style after quitting Prowl,enum=default,enum=compact,enum=none,default=default"`
}
...
// ExitBanner selects what Prowl prints after the TUI exits.
...
type Attribution struct {
	...
	GeneratedWith bool         `json:"generated_with,omitempty" jsonschema:"description=Add Generated with Prowl line to commit messages and issues and PRs,default=true"`
}
...
type Options struct {
	ContextPaths         []string    `json:"context_paths,omitempty" jsonschema:"description=Paths to files containing context information for the AI,example=.cursorrules,example=PROWL.md"`
	GlobalContextPaths   []string    `json:"global_context_paths,omitempty" jsonschema:"description=Paths to files containing global context information for the AI,default=~/.config/prowl/PROWL.md,default=~/.config/AGENTS.md"`
	SkillsPaths          []string    `json:"skills_paths,omitempty" jsonschema:"description=Paths to directories containing Agent Skills (folders with SKILL.md files),example=~/.config/prowl/skills,example=./skills"`
	RulesPaths           []string    `json:"rules_paths,omitempty" jsonschema:"description=Paths to directories containing rule files (*.md) injected as the highest-priority instructions\\, above project context and skills,example=~/.config/prowl/rules,example=./.prowl/rules"`
	TUI                  *TUIOptions `json:"tui,omitempty" jsonschema:"description=Terminal user interface options"`
	Debug                bool        `json:"debug,omitempty" jsonschema:"description=Enable debug logging,default=false"`
	DebugLSP             bool        `json:"debug_lsp,omitempty" jsonschema:"description=Enable debug logging for LSP servers,default=false"`
	DisableAutoSummarize bool        `json:"disable_auto_summarize,omitempty" jsonschema:"description=Disable automatic conversation summarization,default=false"`
	// DataDirectory is where Prowl keeps per-project state such as
...
// Config holds the configuration for prowl.
type Config struct {
