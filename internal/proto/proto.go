...
// UpdateAvailable is published when a newer Prowl release is detected
...
// completion contract (notably `prowl run` in client/server mode)
...
// `prowl run`, which may attach to a busy session whose currently
