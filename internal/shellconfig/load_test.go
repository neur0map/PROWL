...
func TestLoadShellConfig_Provider(t *testing.T) {
	...
	path := filepath.Join(dir, "prowlrc")
...
func TestLoadShellConfig_Option(t *testing.T) {
	...
	dir := t.TempDir()
	script := `option data-directory .prowl
...
option debug`
	...
	opts := result["options"].(map[string]any)
	require.Equal(t, ".prowl", opts["data_directory"])
...
// TestLoadShellConfig_ProwlVersionEnv verifies that PROWL_VERSION is exposed
// to the script so it can feature-detect the running Prowl version.
func TestLoadShellConfig_ProwlVersionEnv(t *testing.T) {
	dir := t.TempDir()
	script := `provider add openai --api-key "$PROWL_VERSION"`
	path := filepath.Join(dir, "prowlrc")
...
func TestLoadShellConfig_ProviderJSONFlagsRequireObjects(t *testing.T) {
	...
	path := filepath.Join(t.TempDir(), "prowlrc")
...
// prowlrc cannot block config loading indefinitely. Config loads run on the
...
func TestLoadShellConfig_RespectsContextCancellation(t *testing.T) {
	...
	select {
	case err := <-done:
		require.Error(t, err, "a cancelled prowlrc must fail, not succeed")
