...
// loadTimeout bounds a single prowlrc execution. Config loading runs on the
...
// LoadShellConfig executes a prowlrc script and returns its config as a
...
func LoadShellConfig(ctx context.Context, path string, src []byte) ([]byte, error) {
	...
	// Expose the running Prowl version so scripts can feature-detect, e.g.
	// [[ "$PROWL_VERSION" == "devel" ]] or branch on the release.
	env := append(os.Environ(), "PROWL_VERSION="+version.Version)
