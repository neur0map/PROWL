...
func TestProwlIgnore(t *testing.T) {
	tempDir := t.TempDir()
	t.Chdir(tempDir)
	...
	// Create a .prowlignore file that ignores .log files
	require.NoError(t, os.WriteFile(".prowlignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .prowlignore file
	prowlignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".prowlignore"), []byte(prowlignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .prowlignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .prowlignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .prowlignore in subdir that ignores normal_nested
	subProwlignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".prowlignore"), []byte(subProwlignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .prowlignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .prowlignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .prowlignore")
