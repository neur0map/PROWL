...
func TestBuildEnv(t *testing.T) {
	...
	require.Equal(t, EventPreToolUse, envMap["PROWL_EVENT"])
	require.Equal(t, "bash", envMap["PROWL_TOOL_NAME"])
	require.Equal(t, "sess-1", envMap["PROWL_SESSION_ID"])
	require.Equal(t, "/work", envMap["PROWL_CWD"])
	require.Equal(t, "/project", envMap["PROWL_PROJECT_DIR"])
	require.Equal(t, "ls", envMap["PROWL_TOOL_INPUT_COMMAND"])
	require.Equal(t, "/tmp/f.txt", envMap["PROWL_TOOL_INPUT_FILE_PATH"])

	// Shared Prowl markers must be present so hook-authored scripts can
	// detect they're running under Prowl the same way bash-tool-invoked
	// scripts can.
	require.Equal(t, "1", envMap["PROWL"])
	require.Equal(t, "prowl", envMap["AGENT"])
	require.Equal(t, "prowl", envMap["AI_AGENT"])
}
...
func TestRunnerEnvVarsPropagated(t *testing.T) {
	t.Parallel()
	hookCfg := config.HookConfig{
		Command: `printf '{"decision":"allow","context":"%s"}' "$PROWL_TOOL_NAME"`,
	}
	r := NewRunner([]config.HookConfig{hookCfg}, t.TempDir(), t.TempDir())
	result, err := r.Run(context.Background(), EventPreToolUse, "sess", "bash", `{}`)
	require.NoError(t, err)
...
func TestParseStdoutClaudeCodeFormat(t *testing.T) {
	...
	t.Run("prowl format still works", func(t *testing.T) {
		t.Parallel()
