...
func BuildEnv(eventName, toolName, sessionID, cwd, projectDir, toolInputJSON string) []string {
	env := os.Environ()
	env = append(env, shell.ProwlEnvMarkers()...)
	env = append(
		env,
		fmt.Sprintf("PROWL_EVENT=%s", eventName),
		fmt.Sprintf("PROWL_TOOL_NAME=%s", toolName),
		fmt.Sprintf("PROWL_SESSION_ID=%s", sessionID),
		fmt.Sprintf("PROWL_CWD=%s", cwd),
		fmt.Sprintf("PROWL_PROJECT_DIR=%s", projectDir),
	)

	// Extract tool-specific env vars from the JSON input.
	if toolInputJSON != "" {
		if cmd := gjson.Get(toolInputJSON, "command"); cmd.Exists() {
			env = append(env, fmt.Sprintf("PROWL_TOOL_INPUT_COMMAND=%s", cmd.String()))
		}
		if fp := gjson.Get(toolInputJSON, "file_path"); fp.Exists() {
			env = append(env, fmt.Sprintf("PROWL_TOOL_INPUT_FILE_PATH=%s", fp.String()))
		}
	}
...
// Supports both Prowl format and Claude Code format (hookSpecificOutput).
