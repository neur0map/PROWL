---
name: prowl-hooks
description: Use when the user wants to add, write, debug, or configure a Prowl hook — gating or blocking tool calls, approving or rewriting tool input before execution, injecting context into tool results, or troubleshooting hook behavior in prowl.json.
---

# Prowl Hooks

Hooks are user-defined commands in `prowl.json` that fire at specific points
...
3. Add the entry under `hooks.PreToolUse` in `prowl.json` with the right matcher.
...
- Non-zero exit codes other than 2/49 are logged but don't block — check Prowl logs.
...
Prowl also accepts Claude Code's `hookSpecificOutput` envelope. One intentional
divergence: Prowl treats `updated_input` as shallow-merge, Claude Code replaces.
