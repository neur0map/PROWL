...
// socketDir returns the directory used for the Prowl Unix socket.
...
// returned by [socketDir] and is named prowl-<uid>.sock, falling
// back to prowl.sock when the current uid cannot be determined. If
// the composed path would exceed [maxUnixSocketPathLen] bytes (the
// macOS sun_path limit), we fall back to /tmp/prowl-<uid>.sock so
// the socket remains bindable.
func DefaultHost() string {
	sock := "prowl.sock"
	usr, err := user.Current()
	if err == nil && usr.Uid != "" {
		sock = fmt.Sprintf("prowl-%s.sock", usr.Uid)
	}
...
// Server represents a Prowl server bound to a specific address.
type Server struct {
