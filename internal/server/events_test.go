...
// are what non-interactive clients (e.g. `prowl run`) rely on to
...
// through the SSE envelope. Without these correlators, `prowl run`
...
// "agent failed" (returns non-zero from `prowl run`) from "agent
