...
import (
	...
	"github.com/neur0map/prowl/internal/oauth/callback"
)
...
func Start(ctx context.Context, cfg Config) (*Flow, error) {
	...
	if err != nil || port < 1 || port > 65535 || redirect.Scheme != "http" || redirect.User != nil || redirect.RawQuery != "" || redirect.Fragment != "" || (redirect.Hostname() != "localhost" && redirect.Hostname() != "127.0.0.1") {
	...
	listener, err := (&net.ListenConfig{}).Listen(ctx, "tcp4", net.JoinHostPort("127.0.0.1", redirect.Port()))
	if err != nil {
	...
	go func() { _ = f.server.Serve(listener); cancel() }()
