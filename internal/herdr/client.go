...
// multiplexer. When Prowl runs inside a herdr-managed pane it reports
...
// Client reports Prowl agent state to a running herdr instance.
type Client struct {
	socketPath string
	paneID     string
...
// call from environment variables. Returns nil when Prowl is not
...
// is not strictly greater. Because prowl seeds seq from the wall
// clock at startup (see newFromEnv), a restarted prowl in the same
...
func (c *Client) newRequestLocked(method, idPrefix, state string) reportRequest {
	...
	return reportRequest{
		ID:     fmt.Sprintf("prowl:%s:%d", idPrefix, time.Now().UnixNano()),
		Method: method,
		Params: reportParams{
			PaneID:         c.paneID,
			Source:         "prowl",
