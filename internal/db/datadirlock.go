...
// already in use by another prowl process.
var ErrDataDirLocked = errors.New("data directory already in use by another prowl process")

// dataDirLockFile is the name of the lock file inside the data
// directory. It lives next to prowl.db so users can `ls` and find it.
const dataDirLockFile = "prowl.lock"
...
// {dataDir}/prowl.lock. If the lock is already held by another
...
// PROWL_SKIP_DATADIR_LOCK is set to a truthy value. This is intended
...
func skipDataDirLock() bool {
	v, _ := strconv.ParseBool(os.Getenv("PROWL_SKIP_DATADIR_LOCK"))
	return v
}
