...
func TestEngineSurvivesReopen(t *testing.T) {
	...
	require.NoError(t, err)
	_, _, err = first.Auth().Setup(ctx, "op@example.com", "correct horse battery", first.Auth().MintSetupCode(ctx))
	require.NoError(t, err)
	...
	n, err := second.Auth().UserCount(ctx)
	require.NoError(t, err)
	require.Equal(t, 1, n, "the account must survive a restart")

	_, _, err = second.Auth().Login(ctx, "op@example.com", "correct horse battery")
	require.NoError(t, err, "credentials must still verify after reopen")
}
