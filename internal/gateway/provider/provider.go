...
// only 401/403 confirm a bad credential; every other status — including a 5xx —
// is treated as live, deliberately, because a validate probe is not the place
// to bench a key for quota or an upstream blip. The upstream reason is
// preserved verbatim so the keys page can show why a key is bad.
