...
// Succeeded resets the ladder: a served request means the endpoint recovered,
// so the next failure should start at the bottom again.
func (c *CooldownEngine) Succeeded(key string) {
	var flush func()
	// Registered before the unlock below, so it runs after it.
	defer func() {
		if flush != nil {
