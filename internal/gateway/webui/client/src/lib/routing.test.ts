...
// #469: a model seeded from an enrolled login arrives with source 'custom' but
// a real platform, and a generic key label ("Prowl login") that must never
// name the row. The company that serves it is the platform, not the label.
describe('enrolled-login provenance (#469)', () => {
  const login: R = { platform: 'anthropic', modelId: 'claude-sonnet-4.5', source: 'custom', keyLabel: 'Prowl login' }

  it('names the platform, never the generic key label', () => {
    expect(providerLabel(login)).toBe('Anthropic')
    expect(memberProviderLabel(login, [login])).toBe('Anthropic')
    expect(memberProviderLabel(login, [login])).not.toBe('Prowl login')
  })

  it('still lets a genuine custom endpoint keep its key label', () => {
    const relay: R = { platform: 'custom', modelId: 'qwen', source: 'custom', keyLabel: 'Ollama box' }
    expect(providerLabel(relay)).toBe('Ollama box')
  })
})

describe('platformDisplayLabel (#469)', () => {
  it('maps known slugs to their company name', () => {
    expect(platformDisplayLabel('openai')).toBe('OpenAI')
    expect(platformDisplayLabel('ryoku-hyper')).toBe('Ryoku Hyper')
  })

  it('title-cases an unmapped slug', () => {
    expect(platformDisplayLabel('anthropic')).toBe('Anthropic')
    expect(platformDisplayLabel('some-new-provider')).toBe('Some New Provider')
  })
})
