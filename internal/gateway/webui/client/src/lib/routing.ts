...
export interface FallbackEntry {
  ...
  // Which wallet a call lands on: nothing, per-token billing, or a
...
// Display names for platforms whose slug does not title-case into the company
// name. An enrolled login cannot be named by its key label ("Prowl login" told
// the operator nothing about who serves the request), so its platform is the
// honest source; unlisted platforms fall back to a title-cased slug. (#469)
const platformDisplayNames: Record<string, string> = {
  ...
  'ryoku-hyper': 'Ryoku Hyper',
}

export function platformDisplayLabel(platform: string): string {
  const known = platformDisplayNames[platform]
...
// A genuine custom endpoint shares the 'custom' platform slug, so only its key
// label ("Ollama box") can name the provider. Every other row — a catalog model
// or an enrolled login — is named by its real platform, never by the key label,
// which for a login is generic and would hide who actually serves the request.
export function providerLabel(row: { platform: string; source?: 'catalog' | 'custom'; keyLabel?: string | null }): string {
  if (row.platform === 'custom' && row.source === 'custom' && row.keyLabel && row.keyLabel.trim()) return row.keyLabel
  return platformDisplayLabel(row.platform)
}
