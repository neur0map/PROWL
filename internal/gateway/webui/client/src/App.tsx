...
import ProviderDirPage from '@/pages/ProviderDirPage'
...
// One destination per question: what can serve me (Models), who serves it
// (Providers), how it chooses (Routing), what it did (Activity), and a place
// to try it (Playground). Keys, agents and billing hang off the overflow menu
// because they are visited once, not while working.
const navItems = [
  { to: '/models', labelKey: 'nav.models' },
  { to: '/providers', labelKey: 'nav.providers' },
  { to: '/routing', labelKey: 'nav.routing' },
  { to: '/activity', labelKey: 'nav.activity' },
  { to: '/playground', labelKey: 'nav.playground' },
]

// Fusion is a routing mode rather than a modality, so it stays reachable from
// the Models entry instead of becoming a sixth top-level tab.
const modelItems = [
  { to: '/models', labelKey: 'catalog.title' },
...
function Brand() {
  return (
    <Link to="/" className="flex items-center gap-2 transition-opacity hover:opacity-70">
      ...
      <span className="font-semibold tracking-tight text-sm">Prowl</span>
    </Link>
  )
}
...
// Browsing 400 models or 200 providers is a table job: a 72rem column clipped
// the right-hand columns off-screen while leaving wide empty margins. These
// routes keep the centred column but are allowed to use the screen.
const WIDE_ROUTES = new Set(['/models', '/providers'])
...
function PageContainer({ children }: { children: ReactNode }) {
  ...
  const width = WIDE_ROUTES.has(location.pathname) ? 'max-w-[112rem]' : 'max-w-6xl'
  return (
    <main className={fullBleed ? 'flex min-h-0 flex-1 flex-col' : `mx-auto w-full ${width} px-6 py-8`}>
      {children}
    </main>
  )
}
                ...
                <PageContainer>
                  <PageBoundary>
                    <Routes>
                      ...
                      <Route path="/providers" element={<ProviderDirPage />} />
                      ...
                      {/* The per-modality catalogues became tabs on /models,
                          and the routing controls moved off the model table. */}
                      <Route path="/models/chat" element={<Navigate to="/models?modality=chat" replace />} />
                      <Route path="/models/embeddings" element={<Navigate to="/models?modality=embeddings" replace />} />
                      <Route path="/models/image" element={<Navigate to="/models?modality=image" replace />} />
                      <Route path="/models/video" element={<Navigate to="/models?modality=video" replace />} />
                      <Route path="/models/audio" element={<Navigate to="/models?modality=audio" replace />} />
                      <Route path="/fallback" element={<Navigate to="/routing" replace />} />
                      ...
                      <Route path="/health" element={<Navigate to="/providers" replace />} />
