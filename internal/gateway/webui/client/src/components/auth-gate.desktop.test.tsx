...
// Node 26 exposes its own `localStorage` global that is undefined without
// --localstorage-file, and it shadows jsdom's inside this environment, so
// every reference here goes through `window`.
...
describe('AuthGate inside the desktop shell', () => {
  ...
  it('shows the recovery message, not a password prompt, when the fresh session is rejected too', async () => {
    ...
    expect(container.textContent).toContain('Quit Prowl and open it again')
  })
