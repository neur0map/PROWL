...
export function AddKeyDialog({ open, onOpenChange, initialPlatform, initialPane }: {
  open: boolean
  onOpenChange: (open: boolean) => void
  initialPlatform?: Platform
  // Lands the dialog on a specific pane, for entry points that already know
  // the provider can only be reached as a custom endpoint.
  initialPane?: Pane
}) {
  const { t } = useI18n()
  ...
  if (open !== prevOpen) {
    ...
    else if (open && initialPlatform) setPane('provider')
  }
        ...
        <DialogPopup maxWidth="max-w-3xl">
          ...
          {pane === 'provider' && (
            <AddKeyForm
              ...
              onUseCustomEndpoint={() => setPane('custom')}
            />
          )}
