...
export function GroupHeaderCells({ group, rank, dragHandle, onToggleGroup, allRows, rateUsage }: {
  ...
  const cost = groupCostBadge(group.members)
      ...
      <td className="py-2 pr-3 align-middle">
        <div className="flex items-center gap-1.5 min-w-0">
          <Link to={`/models/chat/${detailId}`} aria-label={t('models.viewProviders')} onClick={e => e.stopPropagation()} className="flex items-center gap-2 flex-wrap text-left min-w-0">
            ...
            {cost && (
              <span title={cost.title} className={`text-[10px] rounded-full px-1.5 py-0.5 tabular-nums ${cost.tone}`}>
                {cost.text}
              </span>
            )}
...
// groupCostBadge labels what a call on this model spends. A subscription is
// already paid for, so it is called out rather than priced; a per-token model
// shows its output price, which is the number that dominates a coding bill.
// Free models get no chip: an absent price is the page's default.
function groupCostBadge(members: Row[]): { text: string; title: string; tone: string } | null {
  if (members.some(m => m.tier === 'subscription')) {
    return {
      text: 'sub',
      title: 'Covered by a subscription you enrolled',
      tone: 'bg-amber-600/15 text-amber-700 dark:bg-amber-400/15 dark:text-amber-400',
    }
  }
  const prices = members
    .map(m => m.paidOutputPerM)
    .filter((v): v is number => typeof v === 'number' && v > 0)
  if (prices.length === 0) return null
  const low = Math.min(...prices)
  return {
    text: low >= 1 ? `$${low.toFixed(low >= 10 ? 0 : 1)}/M` : `$${low.toFixed(2)}/M`,
    title: `From $${low} per million output tokens`,
