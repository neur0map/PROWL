...
type Route struct {
	...
	BaseURL string
	// RPDLimit / TPDLimit are the model's published daily caps, nil when the
	// provider publishes none. They tell the cooldown engine a daily bench is
...
// recordSuccess is the success-side accounting the driver owns: clear the
// model's penalty and the two per-key streaks a served request disproves. Quota
// counters, key health, and token metering are recorded by the dispatcher /
// integration, which know the token counts.
func (f *Failover) recordSuccess(route Route) {
...
func (f *Failover) applyCooldown(route Route, cls ErrorClass, retryAfter time.Duration) {
	...
	case cooldownPayment:
		f.deps.Cooldowns.Bench(route.Platform, route.ModelID, route.KeyID,
			capByCeiling(paymentRequiredCooldown, f.cfg.CooldownCeiling), SourceCredit)
	case cooldownForbidden:
		f.deps.Cooldowns.Bench(route.Platform, route.ModelID, route.KeyID,
			capByCeiling(modelForbiddenCooldown, f.cfg.CooldownCeiling), SourceTier)
	case cooldownDaily:
...
func (f *Failover) exhaustedRetryError(lastError error, attempts []Attempt, ctx exhaustionCtx, maxRetries int) *Exhaustion {
	...
	case everyAttempt(isClass(AttemptAuth)):
		return &Exhaustion{
			...
			Message: fmt.Sprintf("All %d attempted provider key(s) failed authentication%s. The configured upstream API key(s) look invalid or expired; check the provider keys in the dashboard.%s Last error: %s",
...
func (f *Failover) routingExhaustionBody(routeErr error) *Exhaustion {
	...
	if len(diag) == 0 || allConfig {
		msg := fmt.Sprintf("No candidate model has a configured, usable provider key. Add provider API keys in the dashboard. %s", message)
		if len(diag) == 0 {
			msg = fmt.Sprintf("No models are enabled/configured to serve this request. Add provider API keys and enable models in the dashboard. %s", message)
		}
