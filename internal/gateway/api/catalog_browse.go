...
func (s *Server) registerCatalogBrowseRoutes() {
	s.mux.HandleFunc("GET /api/catalog/models", s.RequireSession(s.handleCatalogModels))
	s.mux.HandleFunc("PUT /api/catalog/use", s.RequireSession(s.handleCatalogUse))
	s.mux.HandleFunc("GET /api/catalog/providers", s.RequireSession(s.handleCatalogProviders))
}
...
func (s *Server) handleCatalogModels(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
...
func (s *Server) catalogChatRows(ctx context.Context, db *sql.DB, keys catalogKeys,
	...
	rowsData, err := db.QueryContext(ctx, `
		SELECT id, platform, model_id, display_name, intelligence_rank, size_label,
		       ...
		       source, paid_input_per_m, paid_output_per_m
	...
	for rowsData.Next() {
		...
		if err := rowsData.Scan(&rw.id, &rw.platform, &rw.modelID, &rw.name, &rw.intelRank,
			...
			&rw.source, &rw.priceIn, &rw.priceOut); err != nil {
			break
		}
	...
	for _, rw := range raws {
		row := catalogRow{
			id: rw.id, platform: rw.platform, modelID: rw.modelID, displayName: rw.name,
		...
		row.hasKey, row.keyStatus, row.usable = catalogKeyState(keys, rw.platform, rw.modelID)
...
func catalogEmbeddingRows(db *sql.DB, keys catalogKeys, perf map[string]pmPerf) []catalogRow {
	rows, err := db.Query(`SELECT id, platform, model_id, display_name FROM embedding_models WHERE enabled = 1`)
	if err != nil {
		return nil
	}
	...
	for rows.Next() {
		var row catalogRow
		row.tier = "free"
		if err := rows.Scan(&row.id, &row.platform, &row.modelID, &row.displayName); err != nil {
			return out
		}
		row.hasKey, row.keyStatus, row.usable = catalogKeyState(keys, row.platform, row.modelID)
		catalogApplyPerf(&row, perf)
		out = append(out, row)
	}
	return out
...
func catalogMediaRows(db *sql.DB, modality string, keys catalogKeys, perf map[string]pmPerf) []catalogRow {
	...
	rows, err := db.Query(
		`SELECT id, platform, model_id, display_name FROM media_models
...
func catalogModalityCounts(db *sql.DB) map[string]int {
	return map[string]int{
		"chat":       catalogGroupCount(db, `SELECT display_name FROM models WHERE enabled = 1`),
...
func (s *Server) handleCatalogUse(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
...
func (s *Server) handleCatalogProviders(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
	...
	modelTotals := catalogModelTotalsByPlatform(db)
	...
	for _, e := range entries {
		...
		routable, inGwCount := 0, 0
		for _, m := range chatByPlatform[platform] {
			if _, _, usable := catalogKeyState(keys, platform, m.modelID); usable {
				routable++
			}
			if inGateway[m.id] {
				inGwCount++
...
func catalogChatModelsByPlatform(db *sql.DB) map[string][]catalogModelRef {
	...
	rows, err := db.Query(`SELECT id, platform, model_id FROM models WHERE enabled = 1`)
	if err != nil {
		return out
	}
	defer func() { _ = rows.Close() }()
...
func catalogModelTotalsByPlatform(db *sql.DB) map[string]int {
	out := map[string]int{}
	...
	add(`SELECT platform, COUNT(*) FROM models WHERE enabled = 1 GROUP BY platform`)
...
func loadCatalogKeys(db *sql.DB) catalogKeys {
	...
	rows, err := db.Query(`SELECT platform, status, enabled, COALESCE(model_scope_json, ''), encrypted_key FROM api_keys`)
	if err != nil {
		return out
	}
	defer func() { _ = rows.Close() }()
	for rows.Next() {
		...
		var enabled int
		if err := rows.Scan(&platform, &status, &enabled, &scopeRaw, &encrypted); err != nil {
			return out
		}
		k := catalogKey{status: status, enabled: enabled != 0}
		if scopeRaw != "" {
...
// catalogKeyState reports the key state for one model: none until an enabled
// key whose scope covers this model exists, then the most usable status found
// (healthy > unknown > error). Usable means routable — an enabled, non-error,
// in-scope key.
func catalogKeyState(keys catalogKeys, platform, modelID string) (bool, string, bool) {
	best, bestRank := "", 0
	for _, k := range keys.byPlatform[platform] {
		if !k.enabled || !catalogScopeAllows(k.scope, modelID) {
			continue
		}
...
func (s *Server) catalogInGatewaySet(db *sql.DB) map[int64]bool {
	if activeID, active := routingActiveProfileID(db); active {
		set, _ := profileMemberSet(db, activeID)
		return set
	}
	set, _ := idSet(db, `SELECT model_db_id FROM fallback_config WHERE enabled = 1`)
	return set
}
