...
// TestProfileTimestampsAreDateStrings covers the other two surfaces that share
...
func TestProfileTimestampsAreDateStrings(t *testing.T) {
	...
	_, body := do(t, s, http.MethodPost, "/api/client-profiles",
		`{"name":"agent","systemPrompt":"be terse"}`, authed(token))
	var profile map[string]any
	require.NoError(t, json.Unmarshal([]byte(body), &profile), "body was %s", body)
	for _, field := range []string{"createdAt", "updatedAt"} {
		value, ok := profile[field].(string)
		require.True(t, ok, "%s must be a date string, got %T", field, profile[field])
