...
func (s *Server) registerRoutingRoutes() {
	s.mux.HandleFunc("GET /api/fallback", s.RequireSession(s.handleFallbackGet))
	s.mux.HandleFunc("PUT /api/fallback", s.RequireSession(s.handleFallbackPut))
	s.mux.HandleFunc("POST /api/fallback/sort/{preset}", s.RequireSession(s.handleFallbackSort))

	s.mux.HandleFunc("GET /api/fallback/routing", s.RequireSession(s.handleRoutingGet))
	s.mux.HandleFunc("PUT /api/fallback/routing", s.RequireSession(s.handleRoutingPut))
	s.mux.HandleFunc("GET /api/fallback/token-usage", s.RequireSession(s.handleTokenUsage))
	s.mux.HandleFunc("GET /api/fallback/rate-limit-usage", s.RequireSession(s.handleRateLimitUsage))
	s.mux.HandleFunc("GET /api/fallback/penalty-inspector", s.RequireSession(s.handlePenaltyInspectorGet))
	s.mux.HandleFunc("DELETE /api/fallback/penalty-inspector", s.RequireSession(s.handlePenaltyInspectorClear))

	s.mux.HandleFunc("GET /api/profiles", s.RequireSession(s.handleProfilesList))
	s.mux.HandleFunc("GET /api/profiles/active", s.RequireSession(s.handleProfileActiveGet))
	s.mux.HandleFunc("POST /api/profiles/active", s.RequireSession(s.handleProfileActiveSet))
	s.mux.HandleFunc("POST /api/profiles", s.RequireSession(s.handleProfileCreate))
	s.mux.HandleFunc("GET /api/profiles/{id}/models", s.RequireSession(s.handleProfileModels))
	s.mux.HandleFunc("PUT /api/profiles/{id}/reorder", s.RequireSession(s.handleProfileReorder))
	s.mux.HandleFunc("DELETE /api/profiles/{id}", s.RequireSession(s.handleProfileDelete))

	s.mux.HandleFunc("GET /api/models", s.RequireSession(s.handleModelsList))
	s.mux.HandleFunc("PATCH /api/models/{id}", s.RequireSession(s.handleModelPatch))
	s.mux.HandleFunc("DELETE /api/models/custom/{id}", s.RequireSession(s.handleModelCustomDelete))
	s.mux.HandleFunc("DELETE /api/models/{id}", s.RequireSession(s.handleModelDelete))
}
...
func routingSetSetting(db *sql.DB, key, value string) error {
	_, err := db.Exec(
...
func (s *Server) handleFallbackGet(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
	db := s.engine.DB()
...
func (s *Server) handleFallbackPut(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
	...
	if active {
		...
		for _, e := range entries {
			...
			} else if _, err := del.Exec(activeID, e.ModelDBID); err != nil {
				WriteError(w, http.StatusInternalServerError, TypeServer, "could not write the chain")
				return
			}
		}
...
// (fallback.ts:416-460).
func (s *Server) handleFallbackSort(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
...
func (s *Server) handleRoutingGet(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
	ctx := r.Context()
	db := s.engine.DB()
	...
	if strategy != gateway.RoutingPriority {
		wv := weightsToOut(routingDisplayWeights(strategy, custom))
		weights = &wv
	}
	...
	WriteJSON(w, http.StatusOK, routingDataOut{
		...
		ExploreEnabled:       routingBoolSetting(db, settingExploreEnabled, false),
...
func (s *Server) handleRoutingPut(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
	...
	if !ok {
		WriteError(w, http.StatusBadRequest, TypeInvalidRequest,
			"strategy must be one of priority, balanced, smartest, fastest, reliable, custom")
		return
	}
	db := s.engine.DB()

	// Persist the weights before the strategy so the new mode reads the
	// intended vector immediately (fallback.ts:88-98).
	if body.Weights != nil {
		...
		enc, _ := json.Marshal(weightsToOut(normalized))
		if err := routingSetSetting(db, settingCustomWeights, string(enc)); err != nil {
			WriteError(w, http.StatusInternalServerError, TypeServer, "could not save custom weights")
			return
		}
	}
	if err := routingSetSetting(db, settingRoutingStrategy, string(strategy)); err != nil {
		WriteError(w, http.StatusInternalServerError, TypeServer, "could not save the strategy")
		return
	}
	if body.ExploreEnabled != nil {
		_ = routingSetSetting(db, settingExploreEnabled, strconv.FormatBool(*body.ExploreEnabled))
	}
	if body.KeySelectionStrategy != nil {
		if *body.KeySelectionStrategy != "auto" && *body.KeySelectionStrategy != "least-remaining" {
			WriteError(w, http.StatusBadRequest, TypeInvalidRequest,
				"keySelectionStrategy must be auto or least-remaining")
			return
		}
		_ = routingSetSetting(db, settingKeySelectionStrategy, *body.KeySelectionStrategy)
	}
	if body.clearCooldownCeiling {
		_ = routingDeleteSetting(db, settingCooldownCeilingMs)
	} else if body.CooldownCeilingMs != nil {
		if *body.CooldownCeilingMs < minCooldownCeilingMs || *body.CooldownCeilingMs > maxCooldownCeilingMs {
			WriteError(w, http.StatusBadRequest, TypeInvalidRequest,
				"cooldownCeilingMs must be between 60000 (1 minute) and 86400000 (24 hours)")
			return
		}
		_ = routingSetSetting(db, settingCooldownCeilingMs, strconv.FormatInt(*body.CooldownCeilingMs, 10))
	}
	if err := s.applyPeakSettings(db, body); err != nil {
		WriteError(w, http.StatusBadRequest, TypeInvalidRequest, err.Error())
		return
	}
	...
	var weights *weightsOut
	if strategy != gateway.RoutingPriority {
		wv := weightsToOut(routingDisplayWeights(strategy, routingCustomWeights(db)))
		weights = &wv
	}
...
func (s *Server) applyPeakSettings(db *sql.DB, body routingPutBody) error {
	if body.PeakHoursAdjust != nil {
		_ = routingSetSetting(db, settingPeakHoursAdjust, strconv.FormatBool(*body.PeakHoursAdjust))
	}
	for _, hf := range []struct {
		key string
		val *int
	}{{settingPeakStartHour, body.PeakStartHour}, {settingPeakEndHour, body.PeakEndHour}} {
		if hf.val == nil {
			continue
		}
		if *hf.val < 0 || *hf.val > 23 {
			return errPeakHour
		}
		_ = routingSetSetting(db, hf.key, strconv.Itoa(*hf.val))
	}
	if body.PeakTimezone != nil {
		if _, err := time.LoadLocation(*body.PeakTimezone); err != nil {
			return errPeakTimezone
		}
		_ = routingSetSetting(db, settingPeakTimezone, *body.PeakTimezone)
	}
	return nil
}
...
// (fallback.ts:463-552).
func (s *Server) handleTokenUsage(w http.ResponseWriter, _ *http.Request, _ gateway.SessionUser) {
	db := s.engine.DB()
	...
	for rows.Next() {
		...
		used := usage[platform+"\x00"+modelID]
...
func monthlyUsageByModel(db *sql.DB) map[string]int64 {
	...
	rows, err := db.Query(`
		SELECT platform, model_id, COALESCE(SUM(input_tokens + output_tokens), 0)
		FROM requests
		WHERE created_at >= ?
		GROUP BY platform, model_id`, monthStart)
	if err != nil {
		return out
	}
	defer rows.Close()
	for rows.Next() {
		...
		var used int64
		if err := rows.Scan(&platform, &modelID, &used); err == nil {
			out[platform+"\x00"+modelID] = used
		}
	}
	return out
}
...
func (s *Server) handleRateLimitUsage(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
	db := s.engine.DB()
	now := time.Now()
...
func (s *Server) handlePenaltyInspectorGet(w http.ResponseWriter, _ *http.Request, _ gateway.SessionUser) {
	db := s.engine.DB()
	now := time.Now()
...
// is zero.
func (s *Server) handlePenaltyInspectorClear(w http.ResponseWriter, _ *http.Request, _ gateway.SessionUser) {
	db := s.engine.DB()
	var cooldowns int64
...
func (s *Server) handleProfilesList(w http.ResponseWriter, _ *http.Request, _ gateway.SessionUser) {
	rows, err := s.engine.DB().Query(`
		SELECT id, name, created_at FROM profiles
		ORDER BY (CASE WHEN LOWER(name) = 'default' THEN 1 ELSE 0 END) DESC, id ASC`)
	if err != nil {
	...
	for rows.Next() {
		...
		out = append(out, profileToOut(id, name, createdAt))
	}
	WriteJSON(w, http.StatusOK, out)
}

func (s *Server) handleProfileActiveGet(w http.ResponseWriter, _ *http.Request, _ gateway.SessionUser) {
...
// (profiles.ts:84-105).
func (s *Server) handleProfileActiveSet(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
...
func (s *Server) handleProfileModels(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
	...
	rows, err := db.Query(`
		SELECT pm.model_db_id, pm.position, m.platform, m.model_id, m.display_name,
	...
	for rows.Next() {
		...
		if err := rows.Scan(&m.ModelDBID, &m.Priority, &m.Platform, &m.ModelID, &m.DisplayName,
			&m.IntelligenceRank, &m.SpeedRank, &m.SizeLabel, &rpm, &rpd, &tpm, &tpd, &m.MonthlyBudget); err != nil {
			WriteError(w, http.StatusInternalServerError, TypeServer, "could not read profile models")
			return
		}
		m.Enabled = true // membership IS the enabled state in this schema
...
// (profiles.ts:138-203).
func (s *Server) handleProfileCreate(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
...
// (profiles.ts:270-295).
func (s *Server) handleProfileReorder(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
	id, ok := routingPathID(w, r)
	if !ok {
		return
	}
...
func (s *Server) handleProfileDelete(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
	id, ok := routingPathID(w, r)
	if !ok {
		return
	}
...
// availability, and key count. The dashboard derives its rows from GET
// /api/fallback; this endpoint stays for API consumers (models.ts:211-283).
func (s *Server) handleModelsList(w http.ResponseWriter, _ *http.Request, _ gateway.SessionUser) {
	db := s.engine.DB()
	activeID, active := routingActiveProfileID(db)
	...
	if active {
		query = `SELECT m.id, m.platform, m.model_id, m.display_name, m.intelligence_rank,
			...
			COALESCE(pm.position, fc.position),
			CASE WHEN pm.model_db_id IS NOT NULL THEN 1 ELSE COALESCE(fc.enabled, 0) END AS fallback_enabled
			FROM models m
			LEFT JOIN profile_models pm ON pm.profile_id = ? AND pm.model_db_id = m.id
			LEFT JOIN fallback_config fc ON fc.model_db_id = m.id
			LEFT JOIN api_keys ak ON ak.id = m.key_id
			ORDER BY COALESCE(pm.position, fc.position, m.intelligence_rank) ASC`
		args = append(args, activeID)
	} else {
		query = `SELECT m.id, m.platform, m.model_id, m.display_name, m.intelligence_rank,
			m.speed_rank, m.size_label, m.rpm_limit, m.rpd_limit, m.tpm_limit, m.tpd_limit,
	...
	for rows.Next() {
		...
		if err := rows.Scan(&m.ID, &m.Platform, &m.ModelID, &m.DisplayName, &m.IntelligenceRank,
			...
			&enabled, &vision, &tools, &m.Source, &keyID, &endpointScope, &keyLabel,
			&pri, &fbEnabled); err != nil {
			WriteError(w, http.StatusInternalServerError, TypeServer, "could not list models")
			return
		}
		m.RPMLimit, m.RPDLimit, m.TPMLimit, m.TPDLimit = nullInt64Ptr(rpm), nullInt64Ptr(rpd), nullInt64Ptr(tpm), nullInt64Ptr(tpd)
...
// handleModelPatch edits a model's catalog fields and/or its chain enable flag.
// Disabling the model drops it from every chain (the chain joins require the
// model be enabled), and fallbackEnabled toggles its global chain row
// (models.ts:102-177).
func (s *Server) handleModelPatch(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
	id, ok := routingPathID(w, r)
	if !ok {
		return
	}
	...
	disablesModel := body.Enabled != nil && !*body.Enabled
	if disablesModel || body.FallbackEnabled != nil {
		next := 0
		if !disablesModel && body.FallbackEnabled != nil && *body.FallbackEnabled {
			next = 1
		}
		if _, err := tx.Exec(`UPDATE fallback_config SET enabled = ? WHERE model_db_id = ?`, next, id); err != nil {
...
// handleModelDelete removes a model and its chain rows. profile_models rows
// cascade on the models foreign key (models.ts:179-208).
func (s *Server) handleModelDelete(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
	id, ok := routingPathID(w, r)
	if !ok {
		return
	}
	...
	WriteJSON(w, http.StatusOK, map[string]any{"success": true, "tombstoned": false})
}

// handleModelCustomDelete removes a custom relay model (models.ts:76-100).
func (s *Server) handleModelCustomDelete(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
	id, ok := routingPathID(w, r)
	if !ok {
		return
	}
