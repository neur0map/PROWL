...
// TestCustomSurfaceRequiresASession keeps the whole custom surface behind the
// one gate that may end a session.
func TestCustomSurfaceRequiresASession(t *testing.T) {
	t.Parallel()
	s := testServer(t, Options{})
	for _, path := range []string{
		...
		require.Equal(t, string(TypeAuthentication), errorType(t, body), path)
	}
}
