...
func seedPresetCatalogue(t *testing.T, s *Server) {
	...
	_, err := s.engine.DB().Exec(`
		...
		VALUES
			('groq','free-big','Free Big',80,60,131072,1,0,1,NULL,'catalog'),
			('groq','free-small','Free Small',40,95,32768,1,0,1,NULL,'catalog'),
			('openrouter','paid-cheap','Paid Cheap',55,90,262144,1,1,1,0.5,'catalog'),
			('openrouter','paid-dear','Paid Dear',95,40,1000000,1,1,1,60.0,'catalog'),
			('anthropic','sub-flagship','Sub Flagship',100,40,200000,1,1,1,25.0,'login'),
			('anthropic','sub-small','Sub Small',45,92,200000,1,0,1,4.0,'login')`)
...
// with the catalogue: the same unchanged rank (40) is in the writing set under
// a low-rank catalogue and out under a high-rank one. A constant threshold
// could never include and exclude the same rank.
func TestPresetWritingCutIsAQuantileNotAConstant(t *testing.T) {
	...
	seedRankedModels(t, low, map[string]int{
		"a10": 10, "a20": 20, "a30": 30, "pivot": 40, "a45": 45, "a50": 50,
	})
	...
	require.True(t, lowSet["pivot"], "rank 40 is top-quartile in the low catalogue")
	require.False(t, highSet["pivot"], "rank 40 is not top-quartile in the high catalogue")
}
