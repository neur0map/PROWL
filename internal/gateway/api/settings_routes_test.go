...
func TestKeyPrefixWidthMatchesTheClientMask(t *testing.T) {
	t.Parallel()
	require.Len(t, unifiedAPIKeyPrefix, 11,
		"the client's fixed-width mask assumes an 11-character prefix")
}
...
func TestMaskedFormNeverLeaksTheKey(t *testing.T) {
	...
	require.True(t, strings.HasPrefix(masked, full[:13]), "the recognisable prefix is preserved")
