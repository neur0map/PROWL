...
func mediaCandidates(ctx context.Context, db *sql.DB, modality, model string) ([]modalityCandidate, error) {
	rows, err := db.QueryContext(ctx, `
		SELECT mm.id, mm.platform, mm.model_id, k.id, k.label, COALESCE(k.base_url, '')
...
func modalityUnavailable(w http.ResponseWriter, modality, model string, hasAnyModel, modelKnown bool) {
	...
	case named:
		WriteErrorCode(w, http.StatusServiceUnavailable, TypeServiceUnavailable, "no_provider_keys",
			fmt.Sprintf("%q exists but no enabled provider key can serve it — add one in the dashboard", model))
	default:
		WriteErrorCode(w, http.StatusServiceUnavailable, TypeServiceUnavailable, "no_provider_keys",
			fmt.Sprintf("no provider key can serve %s yet — add one in the dashboard", modality))
	}
}
...
// recordModalityRequest books a successful non-chat request on the same trail
// the dashboard reads, so embedding and media traffic appears in analytics
// instead of being invisible.
func (s *Server) recordModalityRequest(ctx context.Context, route gateway.Route, inputTokens, outputTokens int, latency time.Duration) {
	if err := s.engine.Vault().MarkHealthyFromRequest(ctx, route.KeyID); err != nil {
		slog.Debug("Could not mark key healthy", "error", err)
	}
	keyID := route.KeyID
...
// useful reading of all, so the error path must not drop it.
func (c *chatRelay) observeQuota(route gateway.Route, err error) {
	var httpErr *provider.HTTPError
	if !errors.As(err, &httpErr) || len(httpErr.Headers) == 0 {
		return
	}
