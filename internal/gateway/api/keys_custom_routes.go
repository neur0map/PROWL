...
func (s *Server) registerKeysCustomRoutes() {
	s.mux.HandleFunc("POST /api/keys/custom", s.RequireSession(s.handleKeysCustomAdd))
	s.mux.HandleFunc("POST /api/keys/custom/probe", s.RequireSession(s.handleKeysCustomProbe))
	s.mux.HandleFunc("POST /api/keys/custom/discover-models", s.RequireSession(s.handleKeysCustomDiscover))
	s.mux.HandleFunc("POST /api/keys/preview", s.RequireSession(s.handleKeysImportPreview))
	s.mux.HandleFunc("POST /api/keys/import-selected", s.RequireSession(s.handleKeysImportSelected))
}
...
func (s *Server) handleKeysCustomAdd(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
	var req struct {
...
func (s *Server) handleKeysCustomDiscover(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
	var req struct {
		BaseURL string `json:"baseUrl"`
		KeyID   *int64 `json:"keyId"`
		APIKey  string `json:"apiKey"`
...
func (s *Server) handleKeysCustomProbe(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
	var req struct {
		BaseURL string `json:"baseUrl"`
		KeyID   *int64 `json:"keyId"`
		APIKey  string `json:"apiKey"`
...
func (s *Server) handleKeysImportPreview(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
...
func (s *Server) handleKeysImportSelected(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
	var req struct {
		Keys []struct {
			KeyName  string `json:"keyName"`
			KeyValue string `json:"keyValue"`
