...
// TestLocalTokenGrantsInferenceOnly is the boundary that keeps a leaked
// inference credential from reconfiguring the gateway.
func TestLocalTokenGrantsInferenceOnly(t *testing.T) {
	t.Parallel()

	s := testServer(t, Options{MachineKey: "prowl-unified-key", LocalToken: "local-bootstrap-token"})

	for _, path := range []string{"/api/keys", "/api/auth/me", "/api/settings/api-key"} {
		resp, body := do(t, s, http.MethodGet, path, "",
			map[string]string{"Authorization": "Bearer local-bootstrap-token"})
		require.Equal(t, http.StatusUnauthorized, resp.StatusCode,
			"%s is a management route and must not accept an inference credential", path)
		require.Equal(t, string(TypeAuthentication), errorType(t, body),
			"%s: a missing dashboard session is the one 401 that may end a session", path)
	}
}
