...
func (s *Server) registerKeysRoutes() {
	...
	s.mux.HandleFunc("GET /api/keys/providers", s.RequireSession(s.handleListProviders))
	s.mux.HandleFunc("POST /api/keys", s.RequireSession(s.handleAddKey))
	s.mux.HandleFunc("POST /api/keys/{id}/reveal", s.RequireSession(s.handleRevealKey))
	...
	s.mux.HandleFunc("PATCH /api/keys/platform/{platform}", s.RequireSession(s.handleTogglePlatform))
	s.mux.HandleFunc("PATCH /api/keys/{id}", s.RequireSession(s.handleUpdateKey))
	s.mux.HandleFunc("DELETE /api/keys/{id}/cooldowns", s.RequireSession(s.handleClearCooldowns))
	s.mux.HandleFunc("DELETE /api/keys/{id}", s.RequireSession(s.handleDeleteKey))

	s.mux.HandleFunc("GET /api/health", s.RequireSession(s.handleHealth))
	s.mux.HandleFunc("POST /api/health/check/{keyId}", s.RequireSession(s.handleCheckKey))
	s.mux.HandleFunc("POST /api/health/check-all", s.RequireSession(s.handleCheckAll))
}
...
func (s *Server) handleListKeys(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
	ctx := r.Context()
...
func (s *Server) handleListProviders(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
	ctx := r.Context()
	db := s.engine.DB()
...
func (s *Server) handleAddKey(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
	var req struct {
		Platform string `json:"platform"`
		Key      string `json:"key"`
		Label    string `json:"label"`
...
func (s *Server) enabledModelCount(ctx context.Context, platform string) int {
	...
	_ = s.engine.DB().QueryRowContext(ctx,
		`SELECT COUNT(*) FROM models WHERE platform = ? AND enabled = 1`, platform).Scan(&n)
	return n
}

// ── PATCH /api/keys/{id} ─────────────────────────────────────────────────────

func (s *Server) handleUpdateKey(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
...
func (s *Server) handleTogglePlatform(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
...
func (s *Server) handleDeleteKey(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
	id, ok := parseIDParam(r, "id")
	if !ok {
		WriteError(w, http.StatusBadRequest, TypeInvalidRequest, "Invalid key ID")
		return
...
func (s *Server) handleRevealKey(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
...
func (s *Server) handleClearCooldowns(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
...
func (s *Server) handleHealth(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
	ctx := r.Context()
	db := s.engine.DB()
	reg := s.engine.Registry()
...
func (s *Server) degradationSnapshot(ctx context.Context) map[string]any {
	...
	snapshot := gateway.HealthSnapshot{Total: total, Healthy: healthy}
...
func (s *Server) handleCheckKey(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
...
func (s *Server) handleCheckAll(w http.ResponseWriter, r *http.Request, _ gateway.SessionUser) {
