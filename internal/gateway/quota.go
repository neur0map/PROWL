...
const (
	...
	// without it one leaked reservation would count against a key forever.
	// Comfortably longer than a per-attempt provider timeout
	// (ratelimit.ts:76 LEASE_MAX_AGE_MS = 2 * MINUTE).
	leaseMaxAge = 2 * time.Minute
...
func (l *Ledger) admitLocked(a Admission, c admitCounts) bool {
	...
	if c.hasMinReqCap || c.hasDailyReqCap || c.hasTokCap {
		poolReq, poolTok := provisionalPool(l.leases, a.Platform, a.KeyID)
		...
		if c.hasTokCap {
			billed := providerBilledTokens(a.Platform, a.TokenMultiplier, poolTok) +
				providerBilledTokens(a.Platform, a.TokenMultiplier, a.EstimatedTokens)
...
func provisionalPool(leases map[int64]*Lease, platform string, keyID int64) (reqs, tokens int64) {
	for _, ls := range leases {
