...
// legacyPlatformAlias maps the pre-port Prowl catalog provider ids (used as
// keys in keys.enc) to the FreeLLMAPI platform strings the registry keys on.
// Every value MUST be a platform the registry knows — registry_test guards
// that. An id absent from this map and unknown to the registry is imported
// under its original id and left DISABLED (never dropped), because a
// credential vanishing on upgrade is the worst outcome of this import.
...
// platform the registry keys on. It is the explicit answer where name-shape
// heuristics fail: "ovhcloud-ai-endpoints" is the adapter "ovh", which no
// prefix rule with a sane minimum length can infer.
func PlatformForCatalogID(id string) (string, bool) {
	platform, ok := legacyPlatformAlias[strings.ToLower(strings.TrimSpace(id))]
	return platform, ok
}
...
func resolveVaultMasterKey(dir string) ([]byte, string, error) {
	...
	case err == nil:
		if len(b) != masterKeySize {
			return nil, "", fmt.Errorf("master key at %s is %d bytes, want %d; restore the original file rather than re-entering keys", path, len(b), masterKeySize)
		}
		return b, "file", nil
...
func (v *KeyVault) CheckKey(ctx context.Context, id int64) (KeyStatus, error) {
	...
	apiKey, derr := v.decryptSecret(enc, iv, tag)
	if derr != nil {
		// An undecryptable row is not a provider verdict; record it and leave
		// the status alone, the same restraint transport errors get.
...
func (v *KeyVault) importLegacyKeys(ctx context.Context) error {
	...
	for _, e := range entries {
		...
		platform, aliased := legacyPlatformAlias[e.Provider]
		if !aliased {
			platform = e.Provider
		}
		...
		// An id neither aliased nor known to the registry is imported DISABLED
		// so the credential is preserved and visible for the operator to fix,
		// never silently dropped.
		if !aliased && !provider.Known(platform) {
			disabled := false
