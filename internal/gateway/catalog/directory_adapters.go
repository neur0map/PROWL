...
// adapterOnlyProviders are keyed by the adapter platform, which is also the
// directory id so the platform lookup resolves without an alias.
var adapterOnlyProviders = []DirectoryEntry{
	{
		ID: "sail", Name: "Sail Research", Class: ClassCredits,
		Friction: "card", APIKeyURL: "https://app.sailresearch.com",
		Note: "$5 of credit each month, and it needs a payment method on file",
	},
	{
...
// withAdapterOnlyProviders appends the entries the upstream lists omit,
...
func withAdapterOnlyProviders(entries []DirectoryEntry) []DirectoryEntry {
	seen := make(map[string]struct{}, len(entries))
	for _, e := range entries {
		seen[e.ID] = struct{}{}
	}
	merged := make([]DirectoryEntry, 0, len(entries)+len(adapterOnlyProviders))
