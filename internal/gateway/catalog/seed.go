...
//     shipped enabled state.
//
// Deliberately absent: pruning and tombstones. The reference needs a
// catalog_model_tombstones table because it syncs from a REMOTE catalog that
// can disagree with the bundled baseline; ours is bundled in the binary and
// changes only when we ship. Its own install carries zero tombstone and zero
// override rows, so that machinery answers a case that does not arise here.
// The consequence, stated plainly because it is the case the tombstone table
// exists to answer: a catalog model the operator DELETES is re-inserted on the
// next boot. The durable way to remove a model from routing is to DISABLE it,
// which survives; deletion is treated as non-durable, exactly as a fresh
// install would carry the model.
//
...
func SeedModels(ctx context.Context, db *sql.DB) (SeedResult, error) {
	...
	for _, m := range data.Chat {
		...
		if _, err := insModel.ExecContext(ctx,
			...
			m.Enabled, m.SupportsVision, m.SupportsTools, m.PaidInputPerM, m.PaidOutputPerM); err != nil {
