	...
	if !opts.SkipCatalogSeed {
		if seeded, err := catalog.SeedModels(ctx, st.DB()); err != nil {
			slog.Error("Gateway model catalog seed failed", "error", err,
				"note", "the dashboard model tabs and catalog will be empty until this succeeds")
		} else {
		...
		if added, err := catalog.EnsureRoutingChain(ctx, st.DB()); err != nil {
			slog.Error("Gateway routing chain seed failed", "error", err,
				"note", "the dashboard model table will be empty until this succeeds")
		} else if added > 0 {
...
// Auth is the dashboard's account and session service.
...
// nothing to route to.
var ErrEngineNotConfigured = errors.New("no provider keys configured yet — open the dashboard and add one")
