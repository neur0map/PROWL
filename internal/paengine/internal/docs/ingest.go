...
func reindex(ctx stdctx.Context, home string) error {
	...
	defer s.Close()
	if _, err = index.IndexWithOptionsContext(ctx, s, dir, index.Options{}); err != nil {
		return err
	}
	// Embed the docs corpus with the dedicated documentation model so docs
	// search is semantic, not just lexical. Embedding is best-effort: on
	// failure the corpus still works via full-text.
	if emb, e := embed.LoadDocs(); e == nil {
		_, _ = index.BuildVectors(ctx, s, emb, emb.EmbedModelID(), index.VectorBudget{}, nil)
	}
	return nil
}
