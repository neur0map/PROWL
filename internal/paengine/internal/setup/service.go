...
import (
	...
	"github.com/neur0map/prowl/internal/paengine/skills"
)
...
const (
	...
	// IntegrationProwl is a user-level-only client (Prowl reads user-level
	// skills, so it needs no project integration). Unlike Hermes, which mirrors
	// Claude's assets, Prowl ships its own native skill tree, installed under
	// ~/.config/prowl/skills/prowl.
	IntegrationProwl = "prowl"
...
// from DetectInstalledHarnesses. Hermes and Prowl have no project-level action
// (their assets are user-level, installed by `prowl-agent skills`), but they
// must survive normalization so a detected ~/.hermes or ~/.config/prowl does
// not fail `init`.
func knownIntegrations() []string {
	return append(append([]string(nil), allIntegrations...), IntegrationHermes, IntegrationProwl)
}
