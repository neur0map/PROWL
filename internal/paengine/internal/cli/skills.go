...
import (
	...
	"github.com/neur0map/prowl/internal/paengine/internal/setup"
)

// newSkillsCmd installs Prowl's agent-native skills under the user's own Claude,
// OMP, Hermes, and Prowl configuration roots. It takes no positional arguments and has
...
func newSkillsCmd(version string) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "skills",
		Short: "Install Prowl's agent-native skills into your Claude, OMP, Hermes, and Prowl config",
		Long: "Install Prowl's release-matched agent skills into your own Claude, OMP,\n" +
			"Hermes, and Prowl configuration roots (~/.claude/skills/prowl, ~/.omp/agent,\n" +
			"~/.hermes/skills/prowl, and ~/.config/prowl/skills/prowl). The command\n" +
			"shows a full preview of every file it\n" +
	...
	cmd.Flags().StringVar(&clientsFlag, "clients", "", "comma-separated clients to target (claude,omp,hermes,prowl); default: detected")
	return cmd
}
...
func renderRestart(out io.Writer, clients []string) {
	...
	for _, client := range clients {
		...
		case setup.IntegrationProwl:
			fmt.Fprintln(out, "  - Reload Prowl so it picks up the prowl agent skills.")
		}
	}
}
