...
func newGateTestCoordinator(t *testing.T, interactive bool) *coordinator {
  ...
  "models": {"large": {"provider": "mock", "model": "mock-model"},
	...
	require.NoError(t, os.WriteFile(filepath.Join(env.workingDir, "prowl.json"), []byte(prowlJSON), 0o644))
...
//
// Non-interactive runs (`prowl run`, both local and client/server) get a single
