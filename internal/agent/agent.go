// Package agent is the core orchestration layer for Prowl AI agents.
//
...
var userAgent = fmt.Sprintf("Ryoku-Prowl/%s (https://github.com/neur0map/PROWL)", version.Version)
...
type SessionAgentCall struct {
	...
	// reliable completion contract (e.g. `prowl run` against a
	...
	// event, so non-interactive clients (e.g. `prowl run`) don't
...
// RunID, e.g. `prowl run`, would otherwise hang). Uncanceled calls without
...
// that RunID: `prowl run` ignores live message events and exits only on a
...
// so callers waiting on those RunIDs (e.g. `prowl run`) are not left
...
// caller waiting on RunComplete (e.g. `prowl run` with a RunID) always
...
func (a *sessionAgent) Run(ctx context.Context, call SessionAgentCall) (result *fantasy.AgentResult, retErr error) {
	...
	if call.Accepted != nil && a.canceledBySeq(call.SessionID, call.Accepted.seq) {
		...
		// `prowl run`, which ignores message events and blocks on
	...
	for {
		...
		history, _ = a.preparePrompt(msgs, largeModel.CatwalkCfg.SupportsImages)
		streamPrompt, files = next, nil
	}
	...
	if err != nil {
		...
		if isCancelErr {
			...
			currentAssistant.AddFinish(message.FinishReasonError, "Unauthorized", `Please re-authenticate with Hyper. You can also run "prowl auth" to re-authenticate.`)
