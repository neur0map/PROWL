...
// event into a closure nobody reads, and subscribers (`prowl run`)
...
// publish a RunComplete for its RunID, hanging a `prowl run` caller that
...
// than internal bookkeeping, which is the contract a `prowl run` caller
...
// bookkeeping slice, not the published event a `prowl run` caller awaits.
