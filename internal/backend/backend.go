...
// Overridable via PROWL_SERVER_IDLE_TIMEOUT (seconds; 0 restores the
...
// via PROWL_SERVER_DETACH_GRACE (seconds; 0 restores immediate teardown).
...
// Backend provides transport-agnostic business logic for the Prowl
...
func New(ctx context.Context, cfg *config.ConfigStore, shutdownFn ShutdownFunc) *Backend {
	return &Backend{
		...
		detachGrace: durationFromEnv("PROWL_SERVER_DETACH_GRACE", DefaultDetachGrace),
	}
}

// idleShutdownDelayFromEnv returns the idle-shutdown delay, honoring a
// PROWL_SERVER_IDLE_TIMEOUT override (in seconds; 0 disables lingering).
func idleShutdownDelayFromEnv() time.Duration {
	return durationFromEnv("PROWL_SERVER_IDLE_TIMEOUT", DefaultIdleShutdownDelay)
}
...
func (b *Backend) CreateWorkspace(args proto.Workspace) (*Workspace, proto.Workspace, error) {
	...
	if err := createDotProwlDir(cfg.Config().Options.DataDirectory); err != nil {
